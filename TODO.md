...
- [x] **CV variants consolidated to Fullstack-only (EN+RU)** — `/cv/`, `/ru/cv/` now serve the former fullstack-positioned content directly; `/cv/frontend/`, `/ru/cv/frontend/` removed entirely; `/cv/fullstack/`, `/ru/cv/fullstack/` kept as thin re-export routes (temporary) pointing at `/cv/` and `/ru/cv/`; `cvContent.ts` reduced from 4 EN/RU content sets to 2; `/es/cv/` temporarily serves EN fullstack content pending an ES rewrite; GymTeam payments bullet reworded (dropped Dolyame, added Robokassa, same 5 providers on EN+RU)
- [x] **PDF text-extraction fixes for ATS parsing** — real `Geist-Italic.woff2` font-face added (was faux/synthetic italic on the context line, causing severe per-character spacing on copy/ATS parse); non-breaking hyphens (U+2011) in `multi‑series`, `rollup‑packaged`, `drag‑and‑drop`, `in‑memory`, `rollup‑пакеты` (regular hyphens were dropped when the word wrapped exactly at the PDF line-break); experience bullets now use literal `•` markers instead of CSS `list-disc` (marker wasn't in the PDF text layer); experience entry field order changed to Company → Role → Period → Location (was Company → Period → Role → Location) so title sits next to company name
