...
- [ ] **Blog — обложки для оставшихся статей** — нужны cover-картинки для: `ai-not-vibe-coding`, `dropped-prod-db`, `pm-defines-solution`; у них нет HTML-слайдов — нужно сделать вручную и конвертировать через `pnpm compress-cover <input> <slug>`
