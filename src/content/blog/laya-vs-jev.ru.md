...
Буквально через пару дней после релиза Jev выяснилось, что он не единственный в своём классе. Появилась другая System One модель, Laya от ConvAI Innovations, и на этот раз open-source. Эту модель можно бесплатно скачать на Hugging Face и запустить у себя.

Напомню, о каком классе моделей идёт речь. Эти модели не генерируют текст и не отдают JSON, они за один проход выдают типизированное решение и показатель уверенности к нему. Например, оценить критичность бага сразу при создании тикета, отфильтровать спам и токсичные комментарии, решить в чат-боте поддержки, может ли он ответить сам по базе знаний или пора звать живого оператора, там, где нужен один конкретный ответ, а не полотно случайного текста.

Всё, что ниже, это разбор по открытым материалам: документация, статьи, чужие примеры и цифры из сравнительных обзоров.

## Что общего

Работают обе по одному и тому же принципу: даёшь модели контекст и вопрос, получаешь в ответ одно из трёх:

- да/нет с вероятностью;
- выбор из заранее заданного списка вариантов, и сразу распределение вероятности по всем из них;
- число по шкале, например критичность бага от 0 до 5.

Один и тот же принцип работы, никакого текста, никаких рассуждений, только голый ответ.

## В чём отличие

Jev доступен только через API, каждый запрос идёт на сервер провайдера и обратно, это 230-280 мс при нормальной сети, но может быть значительно больше, если сеть плохая. Laya запускается локально, на сервере или на ноутбуке, весит около 1 ГБ и может выдавать ответ за 30-40 мс. Разница в 6-7 раз.

За скорость и приватность придётся заплатить точностью и объёмом контекста. Лимит у Laya всего 512 токенов против примерно 4000 у Jev, длинный тикет или письмо туда целиком не поместится.
...
Так или иначе, у System One Models совсем недавно была только одна закрытая модель, а теперь есть и открытая. Её можно посмотреть изнутри и переделать под себя, а не просто вызывать через API вслепую.
